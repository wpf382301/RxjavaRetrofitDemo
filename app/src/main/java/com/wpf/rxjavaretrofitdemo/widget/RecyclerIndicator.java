...
public class RecyclerIndicator extends View {
    ...
    public RecyclerIndicator(Context context, @Nullable AttributeSet attrs, int defStyleAttr) {
        ...
        mListener = new RecyclerView.OnScrollListener() {
            ...
            public void onScrollStateChanged(RecyclerView recyclerView, int newState) {
                if (newState == RecyclerView.SCROLL_STATE_IDLE) {
                    onPageSelected(((LinearLayoutManager)recyclerView.getLayoutManager()).findFirstVisibleItemPosition());
                }
            }
            ...
            public void onScrolled(RecyclerView recyclerView, int dx, int dy) {
                ...
                View viewFirst = recyclerView.getLayoutManager().findViewByPosition(position);
                float width = recyclerView.getWidth();
                if (width != 0 && viewFirst != null) {
                    float right = viewFirst.getRight();
                    float ratio = right / width;
                    onPageScrolled(position, 1 -ratio);
                }
            }
        };
    }
